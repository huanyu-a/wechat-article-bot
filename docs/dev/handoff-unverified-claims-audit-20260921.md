...
| 5 | `/api/health`、登录、8 个业务端点全部 200（§八 19） | **成立** | 用 `admin`/`Admin@123` 登录取 token（`len=43`）后逐条探测：`/api/tasks` `/api/tasks/execution-modes` `/api/dashboard` `/api/articles` `/api/agents` `/api/llm-profiles` `/api/settings/llm` `/api/settings/render` **全部 200**；`/api/health` 200；登录本身 200 | 8/8 命中，与文档所列端点清单完全一致 |
