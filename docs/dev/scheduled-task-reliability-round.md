...
| ③ 效率提升（COORDINATOR / SINGLE） | **不成立**（单样本口径错误 + 模型并不更快） | 本文 §四、§5.3 |
...
**不是效率**——方案「换快模型就能提速」的假设经实测**不成立**（§4.1），且原先的「−15%」是拿最慢的一次
成功运行当基线算出来的（§四）。
...
**关于耗时不能宣称变快**：run#126 比基线 run#116 慢 342 秒。原因已在 §4.1 用实测数据说明——
...
1. **SINGLE 链路仍是「能力优先」档**：run#126 全程用 `glm-5.3-flash`，比基线慢 342 秒（见 §5.3）。
   若要 SINGLE 也快，应把 `builtin_scheduled_creator` 改绑 `deepseek-flash`
   ——**这是唯一被实测证明更快的档**（273 字符/秒 vs glm 的 69~125、hy4 的 77~95）。
...
4. **自然日 09:00 定时触发未观察**（方案第十节第 4 项）：需要等一个自然日，本轮时间窗内没等到。
