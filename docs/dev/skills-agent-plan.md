...
> **写透目标必须用 `findDefault()`，不能用 `defaultProfile()`**（2026-09-17 修复，见 `image-model-per-profile.md` §8.4.4）：
> `defaultProfile()` 是 `findDefault() ?? findFirst()`，那个回落是**读**语义（运行时总得挑一条来用）。
> 拿它当**写**目标时，若表里有档案却没有一行 `is_default=1`，`syncDefaultFromConfig` 会覆盖
> **第一条用户自建档案**且不打默认标记——请求成功、回显也对，但用户的档案（含加密 key）已被静默改写。
> 找不到默认档案时的正确动作是**新建**一条带 `is_default` 的（同名档案则收养并补标记）。
> 同一结论在 `LlmProfileSeeder.defaultSource()` 的 javadoc 里已写过一次，本处曾漏用。
...
| `tool_rec_topic`        | 工具推荐选题与结构 | TOPIC     | 工具推荐类先判定「推荐什么 → 这次要什么 → 谁在读」，再选五套预设组合（快报/共鸣/评测/升级/亲历测评）；含各档篇幅成本、转发理由收敛、时效自检（2026-09-18 新增，见下方「工具推荐技能包」） |
| `tool_rec_title`        | 工具推荐标题公式  | TITLE     | 五类标题公式（降维/提问/评测/升级/亲历）与结构对应；折叠线信息前置；「标题承诺必须在正文兑现」自检（2026-09-18 新增） |
| `tool_rec_opening`      | 工具推荐开头写法  | OPENING   | 五式开头（技术降维/痛点提问/叙事代入/感受描摹/行业痛苦+我选它）与选择规则；首屏折叠线内逐结构规定放什么、不放问候语（2026-09-18 新增） |
| `tool_rec_playbook`     | 工具推荐写作总纲  | WRITING   | 中段总纲：五环节节奏（痛点激活→信任建立→价值展示→顾虑消除→行动触发）+ B/C/D 三模块 + 图文节奏（150-200 字一图）与段落呼吸 + 交付前自检（2026-09-18 新增） |
| `tool_rec_language`     | 工具推荐语言习惯  | LANGUAGE  | 六类金句（翻译/痛点/比喻/认知/爽点/体感）与每 300-400 字一句的插入频率；术语处理；亲历测评语气规范；禁用词与真话原则（2026-09-18 新增） |
| `tool_rec_ending`       | 工具推荐结尾互动  | ENDING    | 五式结尾（人群匹配/态度+行动/反思定性/结构总结/收益定性）与按结尾类型匹配的评论区钩子；价格锚点一致性；禁诱导分享（2026-09-18 新增） |
| `tool_rec_digest`       | 工具推荐摘要写法  | DIGEST    | 摘要公式（工具名+一句话定位+具体收益+适用人群）、40-80 字、不重复标题、必须含一条具体信息（2026-09-18 新增） |
| `tool_rec_compliance`   | 工具推荐合规红线  | FACT_CHECK | 广告法绝对化用语与「免费」边界、效果承诺禁令、利益关系披露（返佣/邀请码）、价格与版本的时效核实、竞品对比可验证、截图真实性与隐私打码、禁诱导分享与破解版；与 `fact_check_default` 叠加生效（2026-09-18 新增） |

（内置 Skill 内容随版本 seed 更新；用户可克隆后自由修改。）

### 附录A-1：「工具推荐」技能包（`tool_rec_*`，2026-09-18 新增）

源自单文件模板库 `~/.workbuddy/skills/wechat-viral-structure/SKILL.md`（约 13k 字：模块化五环节 +
五套预设组合 + 标题公式 + 金句系统 + 转发动机 + 评论区运营）。原文件是**一整份**文档，
绑上去就整份注入；本包按 `SkillPromptAssembler` 的**维度注入**机制拆成 8 枚，
用户只吃自己绑的维度（只写快报文的人不必背上亲历测评的语气规范）。章节到技能的映射：

| 原模板库章节 | 落地技能 | 维度 |
| --- | --- | --- |
| 决策树（选结构）+ 转发动机 + 预设组合 1-5 | `tool_rec_topic` | TOPIC |
| 标题公式 | `tool_rec_title` | TITLE |
| 模块 A + 首屏规则 | `tool_rec_opening` | OPENING |
| 底层逻辑 + 模块 B/C/D + 视觉锚点/段落呼吸 | `tool_rec_playbook` | WRITING |
| 金句系统 + 莫理风语气规范 | `tool_rec_language` | LANGUAGE |
| 模块 E + 评论区钩子 | `tool_rec_ending` | ENDING |
| （原文件缺失）摘要写法 | `tool_rec_digest` | DIGEST |
| （原文件缺失）合规红线 | `tool_rec_compliance` | FACT_CHECK |

两点设计约束（写在这里以免后人重复踩）：

1. **不新增 LAYOUT 种子**：`LAYOUT` 维度在 `SkillPromptAssembler` 里按优先级**单注入**
   （只有优先级最高的一枚生效），本包再塞一枚只会与既有 4 枚版式技能互相打架；
   公众号视觉节奏（150-200 字一图、段落呼吸）因此写在 `tool_rec_playbook` 的「节奏与呼吸」一节里，
   真正的版式仍由用户绑定的 LAYOUT 技能决定。
2. **不新增 IMAGE 种子**：`SkillBindingValidator` 在绑定侧只允许一枚 IMAGE 技能，
   配图风格应继续从既有三枚预设里选，本包不占用该维度。

回归钉子见 `src/test/java/ink/icoding/wechat/article/skill/SkillSeederToolRecommendationTest.java`：
钉住清单与常量一一对应、builtin_key 与 name 全局唯一、维度落在白名单且不占 LAYOUT/IMAGE、
包内容合计 ≤16000 字符，以及各枚技能的关键规则（五套组合、五类标题、五式开头/结尾、六类金句、
合规四条红线）不被后续改写抹掉。

本包**不改变任何内置智能体的默认绑定**（`AgentSeeder` 未改）：它是技能库里的可选项，
由用户在任务/账号/文章上按需绑定，存量任务的行为与现状完全一致（§1.3 第 5 条）。
