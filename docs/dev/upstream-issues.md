...
> **脚本路径要看清楚**：第十四轮起，能进版本控制的探针脚本都搬到了 `tools/render-verify/`（`gen/` 生成器、
> `browser/` 浏览器驱动、`spec/` 输入），本文里凡是写成 `tools/render-verify/...` 的都能 clone 后直接跑。
> 少数早期**一次性排查稿**（`probe_guide_recheck.py`、`probe_containers.py`、`probe_hunt_warn.py`、
> `round8_unknown_tags.py`、`probe_rp_control.py`、`r4_slider_discriminator.py`、`round8_na_discriminator.py`）
> **从未进版本控制**——它们的结论仍然成立（每条都附了产物与数字），但**换台机器无法逐字复现**，
> 本文如实保留原路径、不假装它们还在。
> 采集时间：2026-09-11 ~ 2026-09-14。渲染 API：`POST https://www.bx9y.com.cn/__markflow_render`（header `X-Render-Token`）。

## 条目总览（分级与性质，2026-09-14 第二十轮定稿）

| 编号 | 级别 | 性质 | 一句话 |
| --- | --- | --- | --- |
| §R1 | P1 | **文档缺口** | `guide` 与 Web 组件注册表不一致：8 个容器组件服务端语法指令里一个字都没有 |
| §R2 | P1 | **静默失败** | `:::` 容器类的错误几乎都不报 `meta.warnings`，只能靠产物反推 |
| §R3 | P2 | **渲染缺陷** | 同一个组件在 API 与 Web 导出里的排版不一致 |
| §R4 | P2 | **渲染缺陷** | 标签式 `<slider>` 必须有闭合标签，自闭合写法被当普通正文原样透传 |
| §R5 | P1 | **渲染缺陷** | `Title_DA01` 的 `box-shadow:` 属性名被吃掉 → 卡片没有投影 |
| §R6 | P1 | **渲染缺陷** | `Title_DA01` 的字数是死的：产物里恒为「共 0 字」 |
| §R7 | P2 | **渲染缺陷** | `:::table style="card" title="…"` 的 `title` 属性被丢弃 |
| §R8 | P2 | **渲染缺陷** + 静默 | `:::audience-fit` 静默丢弃第三列（评级），但评级值被消费了 |
| §R9 | P2 | **渲染缺陷** | `:::subscribe` 产出的是假表单：没有 `<input>`、没有 `<button>` |
| §R10 | **P3** | **文档缺口 + 静默失败**（**非渲染缺陷**） | 字段级多行值必须写 `\|` 块标量——文档没写，缺了以后整块静默消失 |

**计数口径**（下面三份文档引用这些数字时必须一致）：
**渲染缺陷共 7 条**：R3、R4、R5、R6、R7、R8、R9（R8 同时属静默失败，按「有渲染缺陷」计入一次）。
若按「用户在编辑器里逐条标注的 11 条中、能复现的上游渲染缺陷」这个更窄的口径，则是 **R5–R9 共 5 条**。
**文档缺口 / 静默失败**另计 **3 条**：R1、R2、R10（R10 已在第二十轮从「渲染缺陷」降级而来）。
两个口径**不是互斥枚举**——R8 同时出现在两个口径里，引用时请写明用的是哪一个。

> 注：`§R4-附` 是**驳回记录**（第五轮误判的 4 条复核后不成立），不是缺陷条目，不计入上面任何一类。
...
- **现象**：`GET /__markflow_render`（无 body）返回的 `guide`（实测 7672 字符）里，块级组件只有编号 1–11 的 11 个标签；而 Web 端组件库实际支持并会用到的这些容器**全文未出现**：
  ...
  > 第十八轮复核（现取实时 guide，7672 字符）：七个名字命中 **0** 次；`align` 命中 **2** 次，但两处都是
  > **`<img>` 的属性**（`align="left"` / 「align 【可选】容器对齐」），**没有一处**是 `:::align` 容器。
  > 即「8 个容器全文未出现」这个结论仍然成立。
...
（脚本 `tools/render-verify/gen/round10_registry_closure.py`，产物 `target/probe/round10_registry_closure.txt`）：
...
第十一轮换了**三条互相独立的证据通道**复核（脚本 `tools/render-verify/gen/round11_crosscheck.py`，
...
python tools/render-verify/gen/component_matrix.py --dump-registry   # → component_registry.json，63 个 ID / 38 个 layout-*
```
...
- **bundle 侧的对应位置**（读 `tools/render-verify/spec/engine/mf_app.js`，**未在浏览器里跑**）：
...
| `<layout-hero>` 标签式不渲染 | `<layout-hero>…</layout-hero>` | **写法不存在**：这个 ID 在 bundle 里**只有注册表条目**（`"layout-hero":"intro"`），没有 `spec`、也没有语法分支 → 容器式 `:::layout-hero` 576 字符正常 | `tools/render-verify/spec/engine/mf_app.js`；样例 `reg-layout-hero` |
...
## 一·续、第十六轮（用户在真实编辑器里逐条标注的 11 条）

> 采集方式：把用户在编辑器里标注的 11 段 MarkFlow 源码**原样**（不改写）打真实渲染 API，
> 生成器 `tools/render-verify/gen/round16_editor_reported.py`，产物 `target/probe/r16/*.html`。
> 下面每一条都在 2026-09-13 用**真实令牌**重打过一次同类最小输入复现（命令见各条「最小复现」）。
> **共 5 条能复现的上游渲染缺陷**（R5–R9；剩下的条目进了本节末尾的「驳回表」）。
> 另有 **§R10（P3）** 一条：它不是渲染缺陷，而是**「文档未覆盖 + 静默失败」**——第二十轮反向自查后
> 从「渲染缺陷」降级，**不主张上游渲染有 bug**（理由见该条正文）。
> 另：这 5 条的响应 `meta.warnings` **全部为 `null`**——与 §R2 同源，符合「错误不报警」的已知模式。
>
> **第二十三轮补记（不影响本节任何条目）**：用户当时是**怎么**把这些内容弄进编辑器的已查清——
> 走的是「打开文章 → `setContent(contentHtml)`」，**不是粘贴、也不是手打**；
> 证据是**用户自己那篇文章 #38 的存库正文**（8 条批注原话写在正文里、这些容器的外层 `<section>` 一层未少、
> 全文 0 处字面 `:::`）。三条路径的逐条实测与判据见 `docs/dev/known-issues-handoff.md` §3.23①。
> **上游侧这 5 条的判定不受路径影响**（它们是在真实渲染 API 的产物上复现的，与用户怎么打开编辑器无关）。

### R5（P1）`Title_DA01` 的投影：`box-shadow:` **属性名被吃掉**，只剩值 → 卡片没有投影

- **现象**：产物里两处卡片/徽标的 `style` 出现**没有属性名**的声明（浏览器整条丢弃）：
  ```
  <section style="margin:0px 0px 30px;rgba(15,23,42,0.05) 0px 10px 24px;border-radius:14px;…">
  <section style="display:inline-block;width:64px;height:64px;…;background-color:#27ae60;rgba(15,23,42,0.16) 0px 12px 24px">
  ```
  前者是标题卡外框、后者是右上角 64×64 的「预计阅读」徽标。两处都**丢了投影**。
- **最小复现**（实测：`box-shadow:` 出现 **0** 次、无名值 **2** 处）：
  ```
  <title type="DA01" label="GUIDE" subtitle="副标题" chips="A|B">测试标题</title>
  ```
- **对照**：同一份 API、同一个字段，`:::table style="card"` 的产物里 `box-shadow:0 4px 16px rgba(0,0,0,0.08),0 1px 4px rgba(0,0,0,0.04)` **完好**（`box-shadow:` 出现 1 次）。
  bundle 里两处模板也都是完整的：`Ke={card:"rgba(15,23,42,0.05) 0px 10px 24px", float:"rgba(15,23,42,0.16) 0px 12px 24px"}`，
  `we.card="margin:0px 0px 30px;box-shadow:rgba(15,23,42,0.05) 0px 10px 24px;border-radius:14px;…"`。
  ⇒ **模板里属性名是在的，是产物里没了。**
- **两处唯一可观察的差异**：标题卡的阴影值**以颜色函数开头**（`rgba(…)` 在前、长度在后），
  而对照组的以长度开头（`0 4px 16px rgba(…)`）。这是目前唯一能把「丢」和「不丢」分开的候选判据，
  但**只是相关、不是已证实的因果**——要坐实得再打一组「值以颜色函数开头 / 以长度开头」的最小对照。
- **期望行为**：属性名不丢；或至少让 `Ke.card` / `Ke.float` 与 `:::table style="card"` 的写法一致。
- **当前绕过**：无（本项目改不动上游模板）。这是用户原话「没有原项目的好看」的可复现成因之一。
- **证据**：`target/probe/r16/r16-06-title-da01.html`；bundle `tools/render-verify/spec/engine/mf_app.js`（`Ke` / `we.card` 字面量）。

### R6（P1）`Title_DA01` 的字数是死的：产物里恒为「共 0 字」

- **现象**：标题卡右下角固定输出 `<p style="…">共 0 字</p>`，同一格的「预计阅读(分)」显示 `1`，
  与输入正文长度无关（最小复现与 40+ 字的真实样例都是 `共 0 字`）。
- **最小复现**：同 R5 的 `<title type="DA01" …>`（`label` / `subtitle` / `chips` 全给了）。
- **期望行为**：`o` 取本次输入的正文长度；API 路径下拿不到正文时，应当**不渲染这一格**而不是渲染 `0`。
- **证据**：`target/probe/r16/r16-06-title-da01.html` 尾部；bundle 模板
  `<p style="${ue.charCount}">${_(\`共 ${o} 字\`)}</p>`。

### R7（P2）`:::table style="card" title="…"` 的 `title` 属性被丢弃

- **现象**：`:::table style="card" title="四种输出模式对比"` 的产物 3717 字符里**没有**「四种输出模式对比」。
  这是第十六轮 11 条里**唯一一条后端判定不达标**的（`must` 判据里列的必有文案缺 1 条）。
- **最小复现**：`:::table style="card" title="四种输出模式对比"` + 两行表格 → 产物里 `title` 文本出现 **0** 次。
- **范围**：本轮只测了 `style="card"` 这一种；**不带 `style="card"` 时 `title=` 是否生效没有测**，
  所以不要把它读成「`:::` 表格的 `title` 一律无效」。
- **期望行为**：`title` 在 `card` 样式下也渲染为表标题；或在 guide 里写明 `card` 不支持 `title`。
- **证据**：`target/probe/r16/r16-09-table-card.html`；live 复现 `tableTitle=false`。
- **旁证**：同一个产物的单元格上还能看到 `;background:#27ae60;;letter-spacing` 这种**双分号**（`th` 两处），
  属同一类模板拼接瑕疵，不单独立项。

### R8（P2）`:::audience-fit` 静默丢弃第三列（评级），但评级值**被消费了**

- **现象**：输入每行三列（人群 `|` 描述 `|` 评级），产物里只有前两列的文字：
  「技术团队　结构严谨、代码块清晰、API 文档可直接复制」——`高` / `中` 在产物里出现 **0** 次。
- **它并不是「整列没读」**：第三行的「中」拿到了琥珀色 `●` 徽标，前两行的「高」拿到绿色 `✓` 徽标，
  即**评级值确实参与了渲染决策，只是从不作为文字输出**。所以这是「丢文本」，不是「丢数据」。
- **最小复现**：`:::audience-fit` + 两行 `技术团队 | 描述一 | 高` / `C 端读者 | 描述二 | 中`
  → live 实测 `rating高=false / rating中=false`。
- **期望行为**：评级作为文字（角标/右侧对齐标签）输出，或明确它在 guide 里只是配色选择器。
- **证据**：`target/probe/r16/r16-05-audience-fit.html`。

### R9（P2）`:::subscribe` 产出的是**假表单**：没有 `<input>`、没有 `<button>`

- **现象**：`:::subscribe` 的产物 1379 字符里 `<input>` **0** 个、`<button>` **0** 个；
  「输入邮箱地址」的占位与「立即订阅」的按钮各自是一个 `<section>` 包一个 `<p>`：
  ```
  <section style="flex:1;min-width:0;padding:10px 14px;background:#fff;border:1px solid #e2e8f0;border-radius:10px;text-align:left"><p …>输入邮箱地址</p></section>
  <section style="flex-shrink:0;padding:10px 18px;background:#27ae60;border-radius:10px;display:flex;…"><p …>立即订阅</p></section>
  ```
- **附带**：副标题与正文的 `<p>` 上同时写了 `text-align:center;text-align:justify`（后者覆盖前者，
  而容器本身已经是 `text-align:center`）——一条自相矛盾的双声明。
- **最小复现**：`:::subscribe` + `title/subtitle/body/placeholder/btn` 五字段 → 产物里 `<input>`/`<button>` 各 0 个。
- **期望行为**：二选一——要么给语义正确的 `<input>` / `<button>`（公众号会剥掉交互，但它至少是可访问的表单）；
  要么承认它是静态卡片，去掉 `/justify` 与表单语义。
- **证据**：`target/probe/r16/r16-02-subscribe.html`。

### R10（P3）字段级多行值**必须**写 `|` 块标量——文档没写，缺了以后整块**静默消失**

> **⚠️ 本条的定义在第二十轮被再次改判过，现在是最终定性：**
> **不是渲染缺陷**，而是**「官方文档未覆盖 + 静默失败」**（P2 → **P3**）。
> 触发改判的是本轮的**反向自查**：把「缺 `|` 就丢 body」当成嫌疑，去证伪「这是设计使然」，
> 结果**证伪失败**——它就是引擎的统一约定（证据在下）。按「宁可撤回重写、不许为了保住条目放宽判据」
> 的口径，本条**降级**，并且**不主张上游渲染有 bug**。

> **怎么找到的**：用户第 10 条报「原项目就没有正确显示」。用**他原文那一种写法**打真实 API
> 复现不出任何缺陷（见驳回表 ⑩）。于是第十八轮改成「换写法再打」，一次试了 10 种，
> 才把这条从「未能复现」变成「换个写法可复现」。第二十轮再做反向自查，把它的定性从
> 「渲染缺陷」压成「文档未覆盖 + 静默失败」。

**两句话结论（先说清 (a) / (b)，避免再被误读）：**

| | 写法 | 真实 API + 真实浏览器 | 结论 |
|---|---|---|---|
| **(a)** | `body: \|` + 缩进（**用户原文用的就是这种**） | 产物 **1558** 字符、body **3/3** 行、`6×6` 圆点 **3** 个、`meta.warnings=null`；真实浏览器参照栏 **14/14** 元素可见、零尺寸节点 **0**、探针「body 三条」「body 圆点」**各 3/3** | **(a) 完全正常**——用户报的那一段，**在他自己给的写法下**是正常的，**复现不出缺陷** |
| **(b)** | `body:` + 缩进（少一个 `\|`）／`body:` + YAML 列表 | 产物掉到 **569** 字符、body **0/3**、圆点 **0**、`meta.warnings` 仍为 `null` | **(b) 确实退化**，但这属于**写法不合引擎约定**，不是 infographic 的渲染缺陷 |

**为什么 (b) 判定为「写法不合约定」而不是「上游缺陷」（反向自查的证伪过程）：**
设想的反例是「`|` 只是 YAML 的块标量语法，缺了它本来就该按字面字符串处理——那 (b) 只是写法不合约定，
不是上游的问题。」为证伪这个反例，本轮做了三组独立测量，**三组都反过来支持「约定如此」**：

1. **是不是 infographic 一个组件的毛病？** 用同一形状的写法打 **4 个带 `body:` 的组件**
   （`infographic` / `verdict` / `notice` / `image-text`），每个各 3 种写法：
   **4 个组件表现完全一致**（inline 单行 = 认；`|` + 缩进 = 认；无 `|` + 缩进 = 整块丢）。
   ⇒ 一条**引擎层面统一的约定**，不是 infographic 的实现缺陷。
   （`target/probe/r16/r10_by_design.json`）
2. **`|` 是不是只对 `body` 生效的硬编码？** 把块标量挪到 **`label` / `title`** 上测：**照样生效**
   （P4/P5 全中）。同时 `>`（折叠）、`|-`（去尾）**也认**（P6/P7），缩进 4 格也认（P8），
   块后面跟非缩进字段能正确收束（P9）。
   ⇒ **字段级通用语法**，不是给 `body` 打的特例。
   （`target/probe/r16/r10_blockscalar.json`）
3. **缩进行会不会被当成字段？** 让正文行里带半角 `:` 与全角 `：`（P2/P3）——**原样保住、没有被
   当成字段键**。⇒ 缩进行是被一个**真正的块标量读取器**整体吃掉的，不是「碰巧混过去」。
   反过来，`body: 首行` 之后另起缩进行（P10）**不认**，进一步说明**续行必须由 `|` / `>` 起头**。

**⇒ 因此 (b) 属于「写法不合引擎的字段级块标量约定」，不是上游渲染有 bug。** 本条**不写成「渲染缺陷」**，
只保留它站得住的那部分：

> ⚠️ 注意：这里说的是 **(b) 这种写法**不合约定，**不是**说用户写错了——**用户给过来的那段是 (a)**，
> 本来就是对的。而**用户当时实际用的到底是哪一种写法，至今不知道**（见本节末尾「尚未确认的一环」）。
> 所以这条**不能**读成「用户少写了一个 `|`」。

- **文档缺口**：官方 guide（存档版 `tools/render-verify/spec/guide_recheck.md` 与实时重取的版本**各 7672 字符**，
  逐字比对只差 2 个字符——上游把「60 套主题」改成了「64 套」）里**搜不到 `infographic` / 信息图**，
  也**搜不到任何「多行值要写 `|`」的说明**（`多行` / `块标量` / `缩进` 命中数均为 **0**；
  `YAML` 仅 1 处，讲的是文章 frontmatter）。用户唯一能照抄的就是引擎包里的内置示例，
  **没有第二处文档告诉他这条规则**——与 §R1 同族。
- **静默失败**：(b) 三种写法的 `meta.warnings` **全是 `null`**，丢的又是**整块正文**（不是降级），
  调用方拿不到任何信号——与 §R2 / §R8 同族。

- **期望行为**（任一即可）：a) 文档里把这条规则写明（引擎内置示例之外再写一处）；
  或 b) 明确拒绝并回报——`meta.warnings` 里给一句「该字段的多行值需要 `|` 块标量」。
- **附带发现（同一支探针顺带测到）**：标签式 `<infographic label=… title=…>` **不被识别**，
  整段标签原样落进正文（产物里字面出现 `<infographic label="读者画像" …>`，包在一个 `<p>` 里）——
  与 §R4 的 `<slider>` 自闭合、以及 38 个 `layout-*` 属同一类「标签式不受支持」。
- **证据**：`target/probe/r16/infographic_variants.json`（10 种写法）、
  `target/probe/r16/r10_by_design.json`（4 组件 × 3 写法）、
  `target/probe/r16/r10_blockscalar.json`（11 组块标量边界）；
  生成脚本 `tools/render-verify/gen/round18_infographic_variants.py`、
  **`tools/render-verify/gen/round20_field_block_scalar.py`**（这两支都已进版本控制——本条结论引用它们，
  按本仓库「被结论引用的脚本不能在 clone 里缺席」的原则；只调渲染 API、不落库）。
- **尚未确认的一环**：用户当时用的究竟是哪一种写法**仍然不知道**，本条**不能**用来解释用户的抱怨。
  他给过来的那段用的是 `body: |`（正常渲染），所以要坐实他遇到的现象，仍需要他给一张出错时的截图
  或原文写法（见驳回表 ⑩ 的结案说明）。

### 第十六轮的**驳回表**（其余条目：上游侧没有可复现缺陷，或缺陷不在上游）

> 与 §R4-附 同样的用途：不让一份「直接转给上游」的清单混进假缺陷。
> 判定口径是「在**真实渲染 API 产物**里能不能复现」，复现不了的就不立条目。

| 用户标注 | 上游侧实际测得 | 归属 | 判定 |
|---|---|---|---|
| ① `:::changelog`「缺少边框」 | 产物容器**有** `border:1px solid #e2e8f0`；1:1 截图里边框画出来了 | 本项目编辑器前端（胶囊 `display` 被重写，已修） | **驳回**：上游无缺陷 |
| ③ `:::author-card`「头像没有渲染完整」 | wrapper `64×64 border-radius:50%`、`img{object-fit:cover}`；源图 `banner4.webp` 实测 **1080×784 横版**，圆形裁切是设计行为 | 上游（源图是横版）+ 编辑器（标签胶囊，已修） | **驳回**：不是渲染缺陷 |
| ④ `:::quote-card`「缺少底色等」 | 产物 `background:linear-gradient(135deg,#fefce8,#fef9c3)`；编辑器往返后渐变元素数 **1→1** 保住 | 本项目编辑器前端（多包一层 `<p>`：这层本身零高，但编辑器给它补了 1.15em 下间距，卡片凭空高 17px；**第二十三轮已修**，高度 145→128＝产物 128，见 §3.23②） | **驳回**：底色没丢 |
| ⑦ `:::summary`「缺少列表符号」 | 产物是 `<span style="width:8px;height:8px;border-radius:50%">` 圆点，结构完整 | 本项目编辑器前端（圆点被内联化，已修） | **驳回**：上游无缺陷 |
| ⑧ `:::checklist`「前边多了一条竖线」 | 产物是 `<span style="width:20px;height:20px;border-radius:6px">` 方框，无竖线元素 | 本项目编辑器前端（方框被内联化 → 塌成竖线，已修） | **驳回**：上游无缺陷 |
| ⑨ `:::table style="card"`「行间距太大 / 整个表太高」 | 产物 `th padding:13px 14px`、`td padding:11px 14px`、只有 **1** 条 `line-height` 声明——**行高不是产物撑开的** | 本项目编辑器前端（`.ProseMirror p{margin:0 0 1.15em}`，见 §3.20③）；改动后余 2 条差异经复验**均非编辑器缺陷**（表头取样错位 + 两栏内容盒差 38px 引起的多折一行，见 §3.20⑤c） | 部分 **驳回**（`title` 丢失另立 R7） |
| ⑩ `:::infographic`「原项目就没有正确显示」 | **用户原文那种写法**（`body: \|`）产物 1558 字符、结构完整，复现不出缺陷；**换写法后确实会退化**——`body:` 缩进少一个 `\|`、或用 YAML 列表，body 三行**全部静默消失**（1558→**569** 字符）。但第二十轮反向自查证明：`\|` 是**字段级通用约定**（`label`/`title` 同样生效，4 个组件表现一致），所以那是**写法不合约定**（见 **§R10**） | 上游（**§R10**，仅限**「文档未覆盖 + 静默失败」**，**不主张渲染 bug**） | 用户原文 **驳回**（复现不出）；邻近写法单独立为 **§R10（P3）**；用户用的具体写法待确认（见下） |
| ⑪ `:::steps-horizontal`「边框没有圆角」 | 产物 `<td style="…border-radius:10px;border:1px solid #eeeeee">` + 表级 `border-collapse:separate;border-spacing:12px 0;min-width:600px` | 本项目编辑器前端（表级声明没进实时 DOM，已修） | **驳回**：上游无缺陷 |

**⑩ 的结案说明（第十八轮改判、第二十轮**再改判定性**）**：用户原话是「原项目就没有正确显示」。
第十六轮只试了**他给的那一种写法**（`body: |`），产物结构完整、复现不出缺陷，于是按 §R4-附 的教训
记为「未能复现、不立项」——**那个判定被推翻了**：第十八轮把 body 的写法换掉重打真实 API，
两种写法下 body 三条正文**整块静默消失**，立为 **§R10**。
第二十轮又对 §R10 本身做反向自查（见 §R10 正文），**把它的定性从「渲染缺陷」压成
「文档未覆盖 + 静默失败」（P3）**：缺 `|` 属于**写法不合引擎的字段级块标量约定**，
不是 infographic 的渲染 bug。
但**用户当时用的是哪一种写法仍然不知道**（他给的那段是 `body: |`，能正常渲染），
所以**本条尚未与他报的那个现象对上号**。

> **一句话结案**：若用户拿不出官网出错截图或当时的原文写法，本条按现有证据记为
> **「用户原写法复现不出缺陷（驳回）；缺 `|` 的邻近写法会整块静默丢正文，属文档未覆盖 + 静默失败
> （§R10，P3，非渲染缺陷）」**，其余不再外推。

**第二十五轮复核：用户的文章 #38 上还残留的 3 条症状，不要转给上游**（结论见 §3.26②）。
同一份输入分别喂**改前 / 改后**的编辑器各逼一次保存出口、逐字节比：
**旧编辑器不丢 changelog 那个外层容器**（`border-radius:12px` / `rgb(226,232,240)` / `1px solid` 计数与改后**完全相同**，
1 / 1 / 2，section 7 个），所以 #38 存库正文里缺它**不是上游缺陷、也不是保存丢的**，
而是**那篇文章的正文停在更早的一次渲染**（存库片段与今天的产物**连文字都不一样**：
`共 2865 字` vs 今天 `共 0 字`、`audience-fit` 那格文案整段不同）。
**唯一坐实在编辑器一侧的是列宽**：改前出口两列都是 `min-width: 25px`，改后出口保留 `width: 90px`
——**旧编辑器会把窄列拍平**（已修），属本项目缺陷、**不入上游清单**。
...
## 四、附：**验收工具自身**的盲区（⚠️ **不是上游缺陷，请勿随上面几条一起转发**）

> 这一节按约定记在这里，但它的**性质与上面完全不同**：R*/A*/G* 是「我们改不动、要对外提」的，
> 这一条是**我们自己那把尺子**的局限。真要转给上游时**请把这一整节删掉**。

### V1（内部，P1）「往返一致性」这一族判据有**结构性盲区**：两边一致地错，它照样判过

- **发现方式**：第二十八轮建「打开 → 保存 → 再打开」往返闸时，用**修复前**的整包前端
  （`target/probe/r26/before-dist`）跑同一套断言做反例自检，**预期是全线判 FAIL**。
- **实测结果（反面）**：`判据① 往返稳定`（保存那一刻的 DOM vs 灌回去之后的 DOM，逐叶子值相同）
  在**修复前的 bundle 上六条样本全部通过、0 条叶子差异**；`判据② 二次往返仍稳定`同样全过。
  真正报出问题的是 `判据③ 入口保真`——恰好 **4 处**失败（`lead-space` / `lead-tab` 段首缩进 0px）。
- **根因**：第二十六轮那个 bug 是「**段首空白在第一次解析时就被吃掉**」。旧前端在「打开」与「再打开」时
  **都**吃、**都**吃得一样多，于是「A 与 B 两次测量自洽」这件事**恒真**。
  **任何只比较「两次测量是否一致」「存进去和读出来是否一样」的断言，对这类 bug 天然免疫。**
- **一般化的教训**（适用于本项目后续所有验收）：
  - 一致性判据只能抓「**单侧不稳定**」（同一圈里前后不一致、或要两圈才收敛）；
  - 抓「**两侧一致地丢**」必须用**入口保真**类判据——比的是「入口声明的量还在不在」，
    而不是「两次测量彼此是否一致」；
  - 因此**建任何一致性闸时，都要先拿「已知坏掉的旧版本」当反例跑一遍**；若旧版本也能过，
    说明这道闸防不住它本该防的那一类，必须补一条入口侧判据。
- **现状**：往返闸（U12）已同时具备判据①/②/③ 与两个反例自检；反向跑退出码 1、前向跑退出码 0。
  复现命令见 `docs/dev/render-verification.md` §3.12 U12，结论全文见
  `docs/dev/known-issues-handoff.md` §3.29①。
