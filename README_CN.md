...
- **创作技能可复用**：把写作风格、排版模板、图片审美和事实核查规则沉淀为带维度标签的技能，按文章/任务/公众号三级绑定注入。
- **智能体与模型档案可配置**：内置智能体的人设与工具组可编辑，可分别指定模型档案；核心工具协议由代码内置，改人设不会破坏工具调用规则。
- **多智能体协作创作**：可选择固定流水线（调研 → 写作 → 配图 → 审核，返工有上限）或由主编智能体自主委托，各阶段共享同一任务工作区。
...
| 创作技能 | 12 个维度（目标读者/选题/写作风格/语言/标题/开头/结尾/摘要/图片/排版/事实核查/其他），内置种子技能，支持三级绑定与注入预览。 |
| 排版引擎 | 指令式（Agent 直接写内联样式 HTML）或渲染式（MarkFlow 语法 Markdown → 渲染服务转版式），语法指令实时获取，支持主题色与图片直链绝对化。 |
| 智能体与模型档案 | 内置 7 个智能体（编辑/定时创作/调研/写作/配图/审核/协调），人设可编辑、工具组可勾选、可绑定专属技能与模型档案。 |
| 执行模式 | 单智能体 / 固定流水线（含返工上限）/ 协调者自主委托，委托次数与返工轮次由代码强制护栏。 |
...
<p align="center">
  <img src="docs/images/skills.png" alt="技能库" width="49%">
  <img src="docs/images/agents.png" alt="智能体" width="49%">
</p>
...
进入 **系统设置 → 排版渲染服务**，接入 MarkFlow 渲染服务（渲染式排版技能依赖此项）：

1. **Base URL**：MarkFlow 渲染服务根地址，默认 `https://www.bx9y.com.cn`。
2. **渲染令牌**：在设置页填写，或用环境变量 `MARKFLOW_RENDER_TOKEN` 注入（环境变量优先，适合 Docker 部署）。
3. **本站公网地址（Site Base URL）**：填写本应用对公网可访问的地址，用于把文章图片 `/uploads/` 相对路径转为渲染服务可访问的绝对直链；不配置时渲染产物中的本站图片可能无法访问。
4. 保存后点击 **测试连接** 验证。不使用渲染式排版技能时无需配置此服务。

> 渲染服务在公网时，部署容器需保证可出网；如需经 HTTP 代理出网，应用侧需显式配置代理（Java HttpClient 不读取系统代理环境变量）。
...
| `MARKFLOW_RENDER_TOKEN` | MarkFlow 渲染服务令牌（优先于设置页存储） | 与 `APP_SECRET_KEY` 同等保密；仅在启用渲染式排版时需要 |
