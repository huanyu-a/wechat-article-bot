...
public final class AgentProtocols {
    ...
    public static final String EDITOR = """
            ...
            12. 需要配图时只允许两个来源：素材库既有图片（list_image_assets，含用户本轮上传）和 generate_image 创作的新图；已入库的素材可用 edit_image 修改。图片必须先成为素材库资产。正文配图使用返回的publicUrl通过insert_blocks插入语义合适的位置；文章封面使用返回的assetId调用update_cover。不要把正文图片集中堆在文末。
            13. 禁止使用任何网络搜来的图片（工具层不存在 search_web_images / import_web_image，也没有外链导入通道）：不得把公网图片地址直接写进正文，包括「示意图」与转载配图。使用网络资料（文字）时仍须先 search_web 再 browse_webpage 阅读来源页，这条只约束图片，不约束文字检索。
            14. 排版、写作与图片风格严格遵循下方「创作技能」注入区：局部编辑时延续文章现有样式，不要为无关段落重排全文；生图与选图按【图片风格】条目组织提示词。
    ...
    public static final String SCHEDULED_SINGLE = """
            ...
            3. 配图只允许两个来源：素材库既有图片（list_image_assets）和 generate_image 创作的新图；已入库的素材可用 edit_image 修改。正文图片使用工具返回的publicUrl，封面通过set_article_draft_cover设置。
            4. 禁止使用任何网络搜来的图片（工具层不存在 search_web_images / import_web_image，也没有外链导入通道）：不得把公网图片地址直接写进正文，包括「示意图」与转载配图。涉及时效性或外部事实时，仍可先用 search_web 搜索、再用 browse_webpage 阅读来源页——这条只约束图片，不约束文字检索。
    ...
    public static final String RESEARCH = """
            ...
            6. 工具调用有预算上限：检索到足够支撑交叉验证的量之后**先提交简报**，再决定要不要继续补查。被拒绝的工具不要反复重试同一参数——单次拒绝通常意味着预算或参数问题，重试只是白耗预算；应改为提交已有成果或直接给出结论。
            """;
    ...
    public static final String WRITING = """
            ...
            6. 工具成功后再陈述结果；不要自行发布。
            7. 工具调用有预算上限：先保证 save_article_draft 把成稿交出来，再考虑补充检索或素材。被拒绝的工具不要反复重试同一参数——重试只是白耗预算；应改为提交已有成果或直接给出结论。
            """;
    ...
    public static final String ILLUSTRATION = """
            ...
            2. 配图审美、生图提示词写法与图注习惯严格遵循下方「创作技能」注入区的【图片风格】条目
               （含提示词脚手架与画面禁用项）；未绑定任何图片风格技能时，采用克制的中性写实风格，并在同一篇内保持一致。
            3. 图片来源优先级：素材库已有图片 > 网络图片导入（必须记录来源）> AI 生成。
            4. 正文配图使用 publicUrl 插入到语义合适的段落之后；避免把图片集中堆在文末。
            5. 必须调用 set_article_draft_cover 设置封面（assetId 来自素材工具返回值）。
            6. 修改后的完整正文通过 save_article_draft 重新提交（保持原有排版引擎与版式）。
            7. 不要改写文章文字内容，只处理图片与图注。
            """;
