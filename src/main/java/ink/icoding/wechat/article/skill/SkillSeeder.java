...
public class SkillSeeder implements ApplicationRunner {
    ...
    static final String MARKFLOW_CONTENT = """
            ...
            2. 组件写法以系统提示中的实时语法指令为准；实测渲染器支持而语法指令未列的有三处，值得记住：步骤用 `:::steps` 容器（每步一个 `###` 小标题，超过 3 步自动竖排）或 `<steps>` 标签；对比用 `:::compare` 容器；提示与注意事项用引用块写 `> [TIP] 内容` / `> [WARNING] 内容`——写成 `:::tip` / `:::warning` 不会被识别，只会退化成普通段落、丢掉提示框样式。
