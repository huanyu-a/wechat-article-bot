...
| `guide_recheck.md` | 渲染服务 `GET /__markflow_render` 返回的官方语法指令（**13744 字节 / 7672 字符**；UTF-8 中文一字三字节，别把字节数当成字符数）。**是给模型看的**，与注册表并不重合——`layout-*` / `gov-header` / `hint` 都不在里面。2026-09-14 与实时版逐字比对：**只差 2 个字符**（存档写「60 套专业配色方案」，线上已改成「64 套」），其余逐字相同 | 交叉验证通道 C | 上游改文案时重取 |
