...
if (ARGS.includes('--selftest')) {
  // 坏输入取**真实存档**，不编数据。
  // ⚠️ 本轮实测：历史喂法假设「粘贴（HTML）」那条路的出口里确实没有段首空白（空白在粘的那一刻被上游折叠），
  // 再把它的「实时DOM量到了缩进」捏成 true，就等价于「DOM 里明明有、保存时丢了」这个真缺陷形态。
  // **该假设对当前真测不再成立**——粘贴入口也接上了窄修法，当前存档里 **5 条路的出口都带着段首空白**。
  // 于是第 2 条改坏改为**真改坏**：把「粘贴（HTML）」那条的出口空白整段抹掉（与第 3 条「手打」同一构造、换一条路），
  // 捏旗子的喂法随历史存档一起定格，不再使用。判据函数（`判`）一字未改。
  ...
  process.exitCode = 自检('27-B r27-entry-paths（判据②：保存出口的段首空白）', 判, [
    { 名: '当前存档（' + 当前.length + ' 条路）', 数据: { 清单: 当前 }, 期望: 0,
      备注: '窄修法在位：5 条路实时 DOM 量到缩进、出口也都带着空白' },
    { 名: '把「粘贴（HTML）」的出口空白整段抹掉（真改坏）', 数据: { 清单: 捏造丢失 }, 期望: 1,
      备注: '「DOM 里明明有、保存时丢了」的真缺陷形态；历史喂法（捏旗子）依赖的「粘贴出口没空白」已随修复落地不成立' },
