...
const empty = () => ({ id:null, name:'', appId:'', appSecret:'', originalId:'', accountType:'SERVICE', verified:true, defaultAuthor:'', defaultStyle:'清爽专业', status:'ACTIVE', skillIds:[] })
...
function open(account) { Object.assign(form, empty(), account || {}); form.appSecret=''; form.skillIds=parseSkillIds(form.skillIds); showForm.value=true }
async function save() { saving.value=true; error.value=''; try { const path=form.id?`/api/accounts/${form.id}`:'/api/accounts'; const payload={...form,skillIds:(form.skillIds||[]).map(Number)}; await api(path,{method:form.id?'PUT':'POST',body:JSON.stringify(payload)}); showForm.value=false; await load() } catch(e){error.value=e.message} finally{saving.value=false} }
